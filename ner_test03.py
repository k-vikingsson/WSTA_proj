...
def parse_docs(doc_set):
    ...
    for i in range (0,no_docs):
        ...
        # if want a list contains entity only:
        # name_entity_str = [" ".join([token for token, tag in ne]) for ne in name_entity]

        # if the end index is needed, use the code below, return entity will be in such format:
        # (sent_id, entity, entity_type, start_index,end_index
        # in most cases, the entity will only contain 1 token, its start_index = end_index
        # name_entity_pairs = [(i," ".join([token for token, tag, start in ne]), ne[0][1],ne[0][2],ne[-1][2]) for ne in name_entity]

        name_entity_pairs = [(i," ".join([token for token, tag, start in ne]), ne[0][1],ne[0][2]) for ne in name_entity]
        # for sent_id,entity,tag,start_i,end_i in name_entity_pairs:
        for sent_id,entity,tag,start_i in name_entity_pairs:
            if tag != 'O':
                if tag == 'ORGANIZATION': 
                    # doc_ne_pairs.append((sent_id,entity,'OTHER',start_i,end_i))
                    doc_ne_pairs.append((sent_id,entity,'OTHER',start_i))

                # code below will fix the end_index for numbers, use will need to return end_index
                ...
                #     doc_ne_pairs.append((sent_id,entity,tag,start_i,end_i))
                    ...
                    # doc_ne_pairs.append((sent_id,entity,'OTHER',start_i,end_i))
                    doc_ne_pairs.append((sent_id,entity,tag,start_i))
    return doc_ne_pairs
...
# sent1 = word_tokenizer.tokenize('There are 20,182 girls in the school.')
